...
const els = {
  ...
  partyStatCurrentList: $("#partyStatCurrentList"),
...
function selectedPartyStatCharacter() {
  const selected = els.partyStatTarget.value || "";
  if (!selected.startsWith("partyIndex:")) return null;
  const index = Number(selected.split(":")[1]) - 1;
  return partyCharacters()[index] || null;
}

function renderPartyStatCurrent() {
  const selected = els.partyStatTarget.value || "current";
  const char = selectedPartyStatCharacter();
  if (!char) {
    const labels = {
    ...
    els.partyStatCurrentList.textContent = [
      `Target mode: ${labels[selected] || selected}`,
      "No saved party summary is attached to this target mode.",
      "Choose a Party slot target to show the party data stored in SaveInfo.json.",
      "Ability scores are applied in-game by the generated Script Extender command."
    ].join("\n");
    return;
  }

  const classes = (char.Classes || []).map((c) => `${c.Main || "Unknown"}${c.Sub ? ` / ${c.Sub}` : ""}`).join(", ");
  const position = Array.isArray(char.Position) ? char.Position.map((v) => Number(v).toFixed(2)).join(", ") : "";
  const lines = [
    `Target: ${els.partyStatTarget.options[els.partyStatTarget.selectedIndex]?.textContent || selected}`,
    `Origin: ${char.Origin || "unknown"}`,
    `Race: ${char.Race || "unknown"}`,
    `Classes: ${classes || "unknown"}`,
    `Level: ${char.Level || "unknown"}`,
    `XP total: ${char["Experience Points (Total)"] || "unknown"}`,
    `XP current level: ${char["Experience Points (Current Level)"] || "unknown"}`,
    `Subregion: ${char.Subregion || "unknown"}`,
    `XYZ: ${position || "unknown"}`,
    "",
    "SaveInfo.json does not expose this character's current STR/DEX/CON/INT/WIS/CHA values here.",
    "Use the fields below to build Script Extender stat boosts for the chosen target."
  ];
  els.partyStatCurrentList.textContent = lines.join("\n");
}
...
els.partyStatTarget.addEventListener("change", renderPartyStatCurrent);
